function x_rts = kf_rts_struct2(yf, zz, Kstruct, Gstruct, G1, Gp, GP)
    
    % 1. dimensions
    fns1 = fieldnames(Kstruct);
    fns2 = fieldnames(Gstruct);
    L = size(G1,1);
    ...
    for p = 1 : P
       ...
       % 1 predictive mean
        ...
        % 5 updated mean
        X_aux = X_aux + Kstruct.(fns1{p})*(yf(:,p) - G * X_aux );
        X_hat(:,p) = X_aux;
    ...
    for p = (P-1) : -1: 1
        ...
        % RTS estimate
%         GG = Gstruct.(fns2{p});
%         GG = GG(:,1:n_atoms_frame);
        %X_first = X_RTS_plus - X_p_minus_plus;
        %X_first = X_first(1:n_atoms_frame,1);
%         X_RTS_all(:,p) = X_p + Gstruct.(fns2{p})*(X_RTS_plus - X_p_minus_plus);
%         X_RTS_all(:,p) = X_p + GG*X_first;
        X_RTS_all(:,p) = X_p + Gstruct.(fns2{p})* ...
            (X_RTS_plus(1:n_atoms_frame,1) - ...
            X_p_minus_plus(1:n_atoms_frame,1));
